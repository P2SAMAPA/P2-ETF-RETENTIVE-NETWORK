...
st.set_page_config(page_title="Retentive Network (RetNet)", layout="wide")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'retnet_' in f]
    if not json_files:
        return None
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the RetNet scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
